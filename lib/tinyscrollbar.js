;(function(window, undefined)
{
    ...
    ,   defaults   =
        ...
        ,   thickness           : false         // Thickness of the scrollbar
        ,   viewportSelector    : '.viewport'   // CSS selector for viewport.
        ,   overviewSelector    : '.overview'   // CSS selector for overview.
        ,   scrollbarSelector   : '.scrollbar'  // CSS selector for scrollbar.
        ,   trackSelector       : '.track'      // CSS selector for track (child of scrollbar).
        ,   thumbSelector       : '.thumb'      // CSS selector for thumb (child of scrollbar).
        }
    ;
    ...
    function Plugin($container, options)
    {
        ...
        ,   $viewport   = $container.querySelectorAll(this.options.viewportSelector)[0]
        ,   $overview   = $container.querySelectorAll(this.options.overviewSelector)[0]
        ,   $scrollbar  = $container.querySelectorAll(this.options.scrollbarSelector)[0]
        ,   $track      = $scrollbar.querySelectorAll(this.options.trackSelector)[0]
        ,   $thumb      = $scrollbar.querySelectorAll(this.options.thumbSelector)[0]
        ...
        function initialize()
        {
            self.toggleAxis(self.options.axis);
            setEvents();

            return self;
        }

        var isHorizontal, sizeLabel, posiLabel, thicknessLabel;
        this.toggleAxis = function(axis) {
            if (!axis) {
              axis = isHorizontal ? 'y' : 'x';
            }
            var scrcls = $scrollbar.className;
            if (isHorizontal !== undefined) { // not the first time
                $thumb.style[posiLabel] = 0;
                $overview.style[posiLabel] = 0;
                $scrollbar.className = scrcls.replace('axis-' + self.options.axis, "");
            }
            this.options.axis = axis;
            isHorizontal = axis === 'x';
            sizeLabel = isHorizontal ? "width" : "height";
            posiLabel = isHorizontal ? "left" : "top";
            thicknessLabel = isHorizontal ? "height" : "width";
            $scrollbar.className = scrcls + ' ' + 'axis-' + axis;
            this.update();
        };
        ...
        function setCss()
        {
            ...
            if (self.options.thickness) {
                $scrollbar.style[thicknessLabel] = self.options.thickness + "px";
                $track.style[thicknessLabel] = self.options.thickness + "px";
                $thumb.style[thicknessLabel] = self.options.thickness + "px";
            }
        }
